...
{
	float sx = 2.f * s[0], sy = 2.f * s[1], sz = 2.f * s[2];
	float x = r[0], y = r[1], z = r[2], w = r[3];
	float n = 1.f / sqrtf(x * x + y * y + z * z + w * w);
	x *= n; y *= n; z *= n; w *= n;
	m[0][0] = sx * (0.5f - (y * y + z * z));
	m[0][1] = sy * (x * y - w * z);
	m[0][2] = sz * (x * z + w * y);
	m[0][3] = t[0];
	m[1][0] = sx * (x * y + w * z);
	m[1][1] = sy * (0.5f - (x * x + z * z));
	m[1][2] = sz * (y * z - w * x);
	m[1][3] = t[1];
	m[2][0] = sx * (x * z - w * y);
	m[2][1] = sy * (y * z + w * x);
	m[2][2] = sz * (0.5f - (x * x + y * y));
	m[2][3] = t[2];
}

// a = b * M
static void vec4_matmul(vec4 a, vec4 b, mat3x4 m)
{
...
{
	mat3x4 m;
	vec4_matmul(m[0], f[0], g);
	vec4_matmul(m[1], f[1], g);
	vec4_matmul(m[2], f[2], g);
...
static void mat3x4_invert(mat3x4 i, mat3x4 m)
{
	...
	n = 1.f / (x * x + y * y + z * z);
	i[0][0] = x * n; i[0][1] = y * n; i[0][2] = z * n; // |i0| = sx / (sx * sx)
